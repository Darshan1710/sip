...
class AffiliateCategory extends CI_Controller {
    ...
    public function categoryList(){
        $data['category'] = $this->AdminModel->getList('affiliate_categories');
        $this->load->view('affiliatecategory/category',$data);
    }
    public function addCategory(){
        ...
        if($this->form_validation->run()){
            ...
            $checkMenu = $this->AdminModel->getDetails('affiliate_categories',$filter);
            if($checkMenu){
                    $returnArr['errCode']                   = 3;
                    ...
                    $addExperience = $this->AdminModel->insert('affiliate_categories',$data);
    ...
    public function editCategory(){
        ...
        if($this->form_validation->run()){
            ...
            $category = $this->AdminModel->getDetails('affiliate_categories',$filter);
            if($category){
                $returnArr['errCode']      = -1;
    ...
    public function updateCategory(){
       ...
       $this->form_validation->set_rules('category','Category Name','required|trim|xss_clean|max_length[255]');
        ...
        if($this->form_validation->run()){
            $filter = array('id !='=>$this->input->post('id'),
                            'category_name'=>$this->input->post('affiliate_categories'));

            $experience = $this->AdminModel->getDetails('affiliate_categories',$filter);
            if($experience){
                    $returnArr['errCode']               = 3;
                ...
                $updateMenu = $this->AdminModel->update('affiliate_categories',$filter,$data);
                if($updateMenu){
                    $returnArr['errCode']     = -1;
    ...
    public function deleteCategory(){
        ...
        if($id){
            ...
            $update = $this->AdminModel->update('affiliate_categories',$filter,$data);
